...
\begin{tikzpicture}[
  font=\footnotesize,
  node distance=4mm,
  block/.style={draw, rounded corners=2pt, minimum height=6mm, minimum width=18mm, align=center, inner sep=2pt},
  frozen/.style={block, fill=gray!12},
  trained/.style={block, fill=green!18, draw=green!50!black},
  arr/.style={-Latex, thick}
]
\node[frozen] (clap) {CLAP audio enc.\\ \emph{frozen}};
\node[frozen, right=12mm of clap] (gpt2) {GPT-2 self-attn\\+MLP \emph{frozen}};
\node[trained, below=4mm of gpt2] (xattn) {Cross-attn\\\emph{trainable}};
\node[block, fill=blue!10, left=8mm of clap] (wav) {raw audio};
\node[block, fill=blue!10, below=4mm of clap] (clap2) {CLAP text\\enc.\\ \emph{frozen}};
\node[block, fill=orange!15, left=4mm of clap2] (store) {Caption\\datastore};
\node[block, fill=orange!15, below=4mm of xattn] (prompt) {Prompt: ``Audios similar\ldots~\textit{c1, c2, c3, c4.}\\This audio sounds like:''};
\node[block, fill=red!10, right=8mm of gpt2] (cap) {output\\caption};

\draw[arr] (wav) -- (clap);
\draw[arr] (clap) -- node[above]{\tiny 64$\times$768} (gpt2);
\draw[arr] (xattn) -- (gpt2);
\draw[arr] (store) -- (clap2);
\draw[arr] (clap2) -- node[right]{\tiny top-$k$} (prompt);
\draw[arr] (prompt) -- (xattn);
\draw[arr] (gpt2) -- (cap);

\begin{scope}[on background layer]
  \node[draw, dashed, rounded corners, fit=(clap2) (store) (prompt), inner sep=2mm, label=below:\tiny retrieval branch] {};
\end{scope}
\end{tikzpicture}
\caption{RECAP architecture. The audio encoder and GPT-2 backbone are frozen;
only the cross-attention sublayers (green) are trained in stage~1. The
retrieval branch (dashed) selects $k$ captions from the AudioCaps training set
using CLAP cosine similarity and pastes them into the prompt fed to the decoder.}
\label{fig:arch}
\end{figure}

\subsection{Retrieval}
Following~\cite{ghosh2024recap}, we score the audio against every training
caption with CLAP cosine similarity. We pre-compute the $\sim 50{,}000\times
512$ caption embedding matrix once and reuse it across all retrieval
configurations. For test queries we exclude any caption whose clip ID matches
the query (a \emph{leakage guard} that the build script enforces by
default).

We compare two retrieval strategies. \textbf{Top-$k$} simply takes the $k$
captions with the highest cosine similarity. \textbf{Maximal Marginal
Relevance (MMR)}~\cite{carbonell1998mmr} re-scores each candidate as
$\lambda \cdot \mathrm{rel}(q,c) - (1-\lambda)\cdot
\max_{c' \in S}\mathrm{sim}(c, c')$, where $S$ is the set already chosen, and
greedily picks $k$ items. With $\lambda=1$ MMR reduces to top-$k$; we use
$\lambda=0.7$, biasing slightly toward diversity.
...
\texttt{data/recap\_dataset.py}); (ii) the retrieval datastore builder with
both top-$k$ and MMR strategies and an explicit train/eval leakage guard
(\texttt{retrieval/}); (iii) a custom GPT-2 cross-attention block that
...
Three patterns are visible in Table~\ref{tab:main}:
...
\toprule
\textbf{Reference (one of 5)} & \textbf{Top retrieved captions ($k{=}4$)} & \textbf{RECAP prediction} \\
\midrule
\emph{a missile launching followed by an explosion and metal screeching as a motor hums in the background} &
``a whoosh and an explosion''; ``a long whistle and then an explosion''; ``a hissing sound followed by a bang''; ``rocket launches and explodes''
& \emph{hissing then an explosion} \\
\addlinespace
\emph{a toy train running as a young boy talks followed by plastic clanking then a child laughing} &
``a child speaking and a man laughing''; ``a young boy talks''; ``a humming machine and a child speaks''; ``a child laughs''
& \emph{a young girl and a man speak before laughing and a woman says something} \\
\addlinespace
\emph{a machine is making clicking sound as people talk in the background} &
``rattling and people speaking''; ``a machine clicks while a man speaks''; ``vehicle engine and people in distance''; ``mechanical clicking''
& \emph{rustling and sputtering with people speaking and wind blowing} \\
\bottomrule
\end{tabular}
\caption{Qualitative outputs of the best stage-1 model on three AudioCaps test
clips. The model copies broad acoustic categories from the retrieved captions
(speech, machinery, explosion) but rephrases freely; it sometimes substitutes
a related-but-wrong sound (``hissing'' for ``missile,'' ``rustling and
sputtering and wind'' for ``rattling clicking'') -- a typical failure mode
when the audio contains a sound class that is rare in the training set.}
\label{tab:qual}
\end{table*}

Table~\ref{tab:qual} shows three test outputs. The model reliably picks up
the dominant acoustic category (speech, machinery, an explosion) and writes
fluent English, but it confuses sounds within a category (``rustling''
vs.\ ``rattling''; ``hissing'' vs.\ ``missile launch''). This is consistent with
CLAP's known weakness on rare event labels and with the observation that
...
caption span. The model overfit modestly in stage~1 (eval loss reached its
minimum around epoch~9 and crept back up by epoch~12), which the
\texttt{load\_best\_model\_at\_end} hook handles. It overfit more aggressively
in stage~2, which is why we cap stage~2 at 10 epochs and use weight decay
$0.01$. We did not measure cross-domain (AudioCaps$\rightarrow$Clotho)
generalisation as originally proposed, so the strongest claims about
generalisation in the original RECAP paper~\cite{ghosh2024recap} are
\emph{not} something we replicate.
